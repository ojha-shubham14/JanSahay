...
export function ConversationMode({
  ...
  /*
   * Maps JanSahay's language selection to
   * Indian English/Hindi/Kannada/Tamil speech recognition.
   */
  ...
  const startListening = () => {
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      if (event.error === 'not-allowed') {
        ...
        setVoiceError('I could not hear anything. Please try again.');
      } else {
        setVoiceError(
  ...
  return (
    <div
      className="card animate-slide-up flex flex-col"
      ...
      >
        ...
        {isProcessing && (
          <div className="flex items-center gap-2 text-sm text-slate-400 animate-fade-in">
            ...
            <span className="px-4 py-2 rounded-2xl bg-slate-100 rounded-tl-sm">
              {tr('chatThinking')}
            </span>
          </div>
        )}
